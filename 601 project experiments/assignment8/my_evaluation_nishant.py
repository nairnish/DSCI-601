...
class my_evaluation:
    ...
    def confusion(self):
        ...
        for label in self.classes_:
            ...
            tn=0
            fn=0
            for l in range(len(self.actuals)):
                if self.actuals[l] == label and self.predictions[l] == label:
                    tp = tp + 1
                elif self.actuals[l] != label and self.predictions[l] == label:
                    fp = fp + 1
                elif self.actuals[l] == label and self.predictions[l] != label:
                    fn = fn + 1
                elif self.actuals[l] != label and self.predictions[l] != label:
            ...
            if average == "micro":
                ...
                for label in self.classes_:
                    ...
                    if prec + rec == 0:
                        ...
                        f1_score_label = 2 * ((prec*rec) / (prec + rec))
                    if average == "macro":
                        ratio = 1 / len(self.classes_)
                    elif average == "weighted":
                        ratio = Counter(self.actuals)[label] / float(n)
            ...
            if target in self.classes_:
                ...
                for i in order:
                    if self.actuals[i] == target:
                        ...
                        pre_fpr = fpr
                        fpr = fp / (fp + tn)
                        auc_target = auc_target + ((fpr - pre_fpr) * (tpr))
            else:
