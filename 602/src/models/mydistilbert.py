...
#Using distilbert multilingual cased trained model as model checkpoint
model_checkpoint = "distilbert-base-multilingual-cased"

sentences = df_train["text"].values
tokenizer = DistilBertTokenizer.from_pretrained(model_checkpoint,use_fast=True)
...
for sent in sentences1:
    ...
    # Add the encoded sentence to the list.
    input_ids1.append(encoded_sent)
...
for sent in input_ids1:
    ...
    # Store the attention mask for this sentence.
    attention_masks2.append(att_mask)

validation_masks = attention_masks2

train_inputs = pd.DataFrame(input_ids)
train_labels = df_train["language"]
validation_inputs = pd.DataFrame(input_ids1)
validation_labels = df_val["language"]


train_inputs = torch.tensor(np.array(train_inputs))
map1 = {'pt-BR':0,'pt-PT':1}
train_labels = train_labels.map(map1)
validation_labels = validation_labels.map(map1)
