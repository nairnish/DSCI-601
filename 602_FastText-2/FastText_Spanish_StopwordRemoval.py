...
def stopwords_removal(df):
    ...
    #Selecting only the Portuguese Variant
    df = df[df['language'].isin(['es-ES','es-PE','es-AR'])]
    return df
...
df_train = convert_data(df_train,"FastText_Spanish_StopwordsRemoval_train.txt")
df_test = preprocess(test_path)
df_test = stopwords_removal(df_test)
df_test = convert_data(df_test,"FastText_spanish_StopwordsRemoval_test.txt")
...
start = time.time()
fastText_train_predict("FastText_Spanish_StopwordsRemoval_train.txt","FastText_Spanish_StopwordsRemoval_test.txt")
end = time.time()
